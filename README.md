...
- `results/` — каталог для артефактов полного публикационного прогона. Эти файлы генерируются после выполнения полного эксперимента и не включены в текущую версию репозитория.
- `results_smoke/` — каталог для результатов коротких технических и smoke-прогонов.
